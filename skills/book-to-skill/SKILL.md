...
`b2s-{author-lastname}-{core-concept}` — e.g. `b2s-cialdini-influence`
...
All book-to-skill output uses a `b2s-` prefix on the skill slug (mirrors this project's `prd-*` naming convention, groups every book skill together in `/skills list`). If `SKILL_NAME` was provided, strip any `b2s-` the user already typed and re-prefix it. Otherwise, propose two options and let the user choose:
- **By author-concept**: `b2s-{author-lastname}-{core-concept}` (e.g. `b2s-cialdini-influence`, `b2s-meadows-systems`)
- **By title**: `b2s-` + lowercase hyphens from book title (e.g. `b2s-designing-data-intensive-apps`)

Default to author-concept format if the book has a strong methodological identity. The slash command to load the skill is always `/b2s-<rest-of-slug>` — say this back to the user after the skill is generated.
