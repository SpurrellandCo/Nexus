...
When the user wants to **implement** SEO fixes, produce this brief before spawning:

**When:** User says "fix these issues", "implement the recommendations", "apply these changes"
**Agent:** `coder` (use `backend-dev` if fixes involve server config — redirects, headers, sitemap generation)

Brief to produce:
```
Task: Implement SEO fixes from audit
Agent: coder
Files: [layout/head file for meta changes; sitemap.ts/xml for sitemap; robots.txt for crawl rules]
Changes:
  - [specific fix 1 — e.g. "Add canonical tag to /blog/* pages"]
  - [specific fix 2 — e.g. "Set meta description on homepage to: [text]"]
  - [specific fix 3]
