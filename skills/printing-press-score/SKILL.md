...
After rendering the score output, check the total score:

- **Score ≥ 85 (Grade A)** — CLI is ship-ready. Offer `/printing-press-publish <name>` as the next step.
- **Score < 85 (Grade B or below)** — surface the top failing dimensions, then ask:

  > "Score is below Grade A. Run `/printing-press-polish` to automatically fix the top issues?"

  If the user confirms, invoke the `printing-press-polish` skill passing the same CLI name/directory that was scored. Polish re-scores at the end of its run so the user sees the improvement delta without needing to re-run `/printing-press-score` manually.
