...
When the user wants to **implement** signup flow changes, produce this brief before spawning:

**When:** User says "implement this", "update the form", "make these changes to signup"
**Agent:** `coder` (use `backend-dev` if changes involve validation logic, auth routes, or DB fields)

Brief to produce:
```
Task: Implement signup flow changes
Agent: coder
Files: [signup page/component, form component, auth route if backend changes needed]
Changes:
  - [specific change 1 — e.g. "Remove phone number field from SignupForm.tsx"]
  - [specific change 2 — e.g. "Add social proof line below email field: [text]"]
  - [specific change 3]
