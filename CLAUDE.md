...
**Shared with Codex + Gemini CLI** (added 2026-09-24): `scripts/nexus-link.js` links every skill into `~/.agents/skills` and renders agents into `~/.codex/agents` / `~/.gemini/agents` (Stop hook `nexus-link-auto.js`, `update.sh`, nightly sync, install). Never copy skills or agents into other tools by hand. New/edited skills and agents are linted by `hooks/portability-check.js`; conventions in `rules/nexus/portable-authoring.md`. Which tools get them: `shareWith` in `~/.nexus-local/config.json`. Snapshot/verify the whole setup with `node ~/.claude/scripts/nexus-baseline.js [--compare <file>]` (Phase 0 baseline: `plans/baseline-2026-09-24.json`).

## Keeping Nexus Updated

Nexus is a git checkout at `~/.nexus` (moved out of `~/.claude` on 2026-09-24). Everything it provides appears in `~/.claude` as symlinks (`~/.claude/skills -> ~/.nexus/skills`, same for agents, rules, commands, scripts, hooks, `CLAUDE.md`, ...), kept up to date by `scripts/nexus-claude-links.sh`; Claude Code's own runtime files (`settings.json`, `projects/`, `sessions/`, `plugins/`) stay real files in `~/.claude`. Run git commands in `~/.nexus`, not `~/.claude`. If asked to check for, pull, or apply updates to this config (in any phrasing — "pull the updates," "is there a new version," "update my setup"), run `bash ~/.nexus/update.sh` (or `/nexus-update`) rather than a bare `git pull` — it also refreshes machine-level dependencies afterward and reports what changed. Nexus itself only pushes new commits once a day via a scheduled `nexus-daily-sync.sh` job (not in real time), so "no updates yet" on a same-day check is expected, not a bug. The sync commits only Nexus's own paths (the `SYNC_PATHS` allowlist in the script; Claude caches like `plugins/`, `state/`, `skills/synced/` are gitignored and never committed) and pushes only when `sync.push` is true in `~/.nexus-local/config.json` (true on this machine). The repo is public, so anything outside the allowlist stays local.
